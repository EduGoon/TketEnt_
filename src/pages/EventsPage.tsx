...
const EventsPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      // 2. Fetch the favorites to restore the stars
      try {
        const resp = await eventService.listFavorites();
        const favData = resp.data || resp; // Handle both {data: []} and [] patterns
        
        if (Array.isArray(favData)) {
          const map: { [key: string]: boolean } = {};
          favData.forEach((f: any) => {
            // Use f.eventId (from your Favorite model) or f.id
    ...
    <div style={{ minHeight: '100vh', background: '#0a0d14', color: '#fff', fontFamily: "'DM Sans', 'Helvetica Neue', sans-serif" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .fav-btn.active { background: rgba(240,192,64,0.15); border-color: rgba(240,192,64,0.4); color: #f0c040; }
        .fav-btn.inactive { background: rgba(255,255,255,0.04); color: rgba(255,255,255,0.4); }
        .fav-btn:hover { border-color: rgba(240,192,64,0.4); color: #f0c040; }
      ...
      <header style={{
        ...
        <div style={{
          ...
          <span style={{
            fontSize: 17, fontWeight: 700, color: '#f0c040',
      ...
      <div style={{ maxWidth: 1060, margin: '0 auto', padding: '52px 24px 36px', animation: 'fadeUp 0.5s ease forwards' }}>
        <p style={{ fontSize: 9, letterSpacing: 4, color: 'rgba(240,192,64,0.65)', textTransform: 'uppercase', fontFamily: "'DM Mono', monospace", marginBottom: 10 }}>
          Discover
        </p>
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <h1 style={{
            ...
            fontSize: 42, fontWeight: 700, lineHeight: 1.1, letterSpacing: -1,
          }}>
        ...
        <div style={{ maxWidth: 1060, margin: '0 auto', padding: '0 24px' }}>
          {loading ? (
            ...
            <div style={{
              ...
              <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, color: 'rgba(255,255,255,0.5)', fontStyle: 'italic', marginBottom: 6 }}>
                No events found.
              </p>
              <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', fontFamily: "'DM Mono', monospace", letterSpacing: 1 }}>
                ...
                {filteredEvents.map((event, idx) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className="ev-card"
                      style={{ animation: `fadeUp 0.45s ease ${idx * 0.06}s both` }}
                    >
                      {/* Image */}
                      <div style={{ position: 'relative', overflow: 'hidden', flexShrink: 0 }}>
                        ...
                        <button
                          ...
                          style={{
                            ...
                            background: isFavorite ? 'rgba(240,192,64,0.9)' : 'rgba(10,13,20,0.7)',
                            border: `1px solid ${isFavorite ? '#f0c040' : 'rgba(255,255,255,0.15)'}`,
                            borderRadius: 8, width: 34, height: 34,
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            fontSize: 15, cursor: 'pointer',
                            backdropFilter: 'blur(8px)',
                            transition: 'all 0.2s',
                            color: isFavorite ? '#0a0d14' : 'rgba(255,255,255,0.7)',
                          }}
                      ...
                      <div style={{ padding: '18px 20px 22px', display: 'flex', flexDirection: 'column', flex: 1 }}>
                        ...
                        <p style={{ fontSize: 12, color: '#60c8f0', marginBottom: 3, fontFamily: "'DM Mono', monospace" }}>
                          📍 {venue}
                        </p>
                        <p style={{ fontSize: 11, color: 'rgba(255,255,255,0.32)', fontFamily: "'DM Mono', monospace", marginBottom: 16 }}>
                          🗓 {eventStart
                            ? new Date(eventStart).toLocaleString(undefined, { weekday: 'short', month: 'short', day: 'numeric', year: 'numeric', hour: '2-digit', minute: '2-digit' })
                            : 'Date TBD'}
                        </p>
                        ...
                        <div style={{
                          ...
                          <Link to={`/events/${event.id}`} className="view-btn">
                            View Details <span className="ev-arrow">→</span>
                          </Link>
                        </div>
                      </div>
                    </div>

...
export default function AdminPayoutsManager() {
  ...
  const handleRelease = async (id: string) => {
    ...
    try {
      ...
      showToast('Payout released — email sent to organizer');
  ...
  return (
    <div>
      {toast && (
        <div className="fixed top-6 right-6 z-50 bg-gray-900 text-white px-5 py-3 rounded-xl text-sm font-medium shadow-xl">{toast}</div>
      )}

      <div className="flex justify-between items-center mb-8">
        <div>
          <h2 className="text-3xl font-bold text-gray-800">Payout Requests</h2>
          <p className="text-gray-500 text-sm mt-1">Review and release organizer payouts</p>
        </div>
              ...
              <div key={p.id} className="bg-white rounded-2xl border border-gray-100 p-6 hover:border-gray-300 transition-all cursor-pointer" onClick={() => { setSelected(p); setAdminNote(''); }}>
                <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-0.5">{p.organizer?.firstName} {p.organizer?.lastName} · {p.organizer?.email}</p>
                    <p className="text-xs text-gray-400 mt-1 font-mono">Requested {new Date(p.requestedAt).toLocaleDateString('default', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                  </div>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelected(null)}>
          ...
          <div className="relative z-10 bg-white rounded-3xl w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            <div className="p-8">
              <div className="flex justify-between items-start mb-6">
                <div>
                  <h3 className="text-xl font-black text-gray-900">{selected.event?.title}</h3>
                  <p className="text-gray-500 text-sm mt-1">{selected.organizer?.firstName} {selected.organizer?.lastName}</p>
                </div>
              ...
              <div className="space-y-3 mb-6">
                {[
                  { label: 'Gross Amount',  value: `KSH ${selected.grossAmount.toLocaleString()}` },
                  { label: 'Platform Fee',  value: `KSH ${selected.platformFee.toLocaleString()}` },
                  { label: 'Net to Pay',    value: `KSH ${selected.netAmount.toLocaleString()}` },
                  { label: 'Organizer Email', value: selected.organizer?.email },
                ].map(row => (
              ...
              {selected.status === 'PENDING' && (
                <>
                  <div className="mb-4">
                    ...
                    <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)} rows={3} placeholder="Add a note…" className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-green-500 resize-none" />
                  </div>
                  <div className="flex gap-3">
                    <button onClick={() => handleRelease(selected.id)} disabled={acting} className="flex-1 bg-green-600 hover:bg-green-700 text-white py-3 rounded-xl font-bold text-sm disabled:opacity-50">
                      {acting ? 'Processing…' : '↑ Release Payout'}
                    </button>
                    <button onClick={() => handleReject(selected.id)} disabled={acting} className="flex-1 bg-red-50 hover:bg-red-100 text-red-600 py-3 rounded-xl font-bold text-sm disabled:opacity-50">
              ...
              {selected.status !== 'PENDING' && (
                <div className={`p-4 rounded-xl text-sm font-medium ${STATUS_COLORS[selected.status].bg} ${STATUS_COLORS[selected.status].text}`}>
                  ...
                  {selected.releasedAt && <p className="mt-1 opacity-60">Released: {new Date(selected.releasedAt).toLocaleDateString()}</p>}
                </div>
              )}
            </div>
          </div>
